...
public class ConvertNegatedExpressionWithDemorgansLawIntention : JetSelfTargetingIntention<JetPrefixExpression>(
    ...
    override fun isApplicableTo(element: JetPrefixExpression): Boolean {
        ...
        val parenthesizedExpression = element.getLastChild() as? JetParenthesizedExpression
        ...
        while (binaryElement != null) { // While loop lets us handle expressions of any length
            val leftChild = binaryElement?.getFirstChild()
            val rightChild = binaryElement?.getLastChild()
            if (leftChild is JetPrefixExpression || rightChild is JetPrefixExpression
            || rightChild is PsiErrorElementImpl || operatorToken != binaryElement?.getOperationToken()) {
                return false
            }
    ...
    override fun applyTo(element: JetPrefixExpression, editor: Editor) {
        ...
        while (binaryElement != null) {
            val leftChild = binaryElement?.getFirstChild()
            val rightChild = binaryElement?.getLastChild()
            val operatorText = when(binaryElement!!.getOperationToken()){
                JetTokens.ANDAND -> "||"
                JetTokens.OROR -> "&&"
                else -> ""
            }
