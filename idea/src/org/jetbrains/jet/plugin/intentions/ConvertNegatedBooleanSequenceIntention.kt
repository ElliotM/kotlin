...
public class ConvertNegatedBooleanSequenceIntention : JetSelfTargetingIntention<JetBinaryExpression>(
    ...
    override fun isApplicableTo(element: JetBinaryExpression): Boolean {
        ...
        do {
            ...
            when {
                ...
                rightChild is PsiErrorElementImpl,
    ...
    override fun applyTo(element: JetBinaryExpression, editor: Editor) {
        ...
        while (binaryExpression != null) {
            val leftChild = binaryExpression!!.getFirstChild()
            val rightChild = binaryExpression!!.getLastChild() as JetPrefixExpression
            val operator = when(binaryExpression!!.getOperationToken()) {
                JetTokens.ANDAND -> "||"
                JetTokens.OROR -> "&&"
                else -> throw IllegalArgumentException() //Should never happen, exception is required for when block type
            }
            expressionText = " $operator ${rightChild.getBaseExpression()!!.getText()}$expressionText"
